      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        >
          ...
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="px-3 py-1 text-sm rounded-lg hover:bg-sage-50 transition-colors text-sage-600"
              >
                Hoje
              </button>
              <button
          ...
          <div className="grid grid-cols-7 gap-2 mb-2">
            {weekDays.map((day) => (
              <div key={day} className="text-center text-sm font-medium text-sage-500 py-2">
                {day}
              </div>
            ))}
          </div>
            ...
            {calendarDays.map((day, index) => {
              ...
              return (
                ...
                >
                  <span className={`
                    text-sm font-medium
                    ${isSelected ? 'text-terracotta-600' : isCurrentMonth ? 'text-sage-700' : 'text-sage-400'}
                    ${isTodayDate ? 'text-sage-700 font-bold' : ''}
                  `}>
                    {format(day, 'd')}
                  </span>
                  ...
                  {dayAppointments.length > 0 && (
                    <div className="mt-1 space-y-1">
                      {dayAppointments.slice(0, 2).map((apt) => (
                        <div
                          key={apt.id}
                          className={`text-xs px-1 py-0.5 rounded truncate text-white ${statusColors[apt.status as keyof typeof statusColors] || statusColors.scheduled}`}
                        >
                          {format(parseISO(apt.date), 'HH:mm')}
                        </div>
                      ))}
                      {dayAppointments.length > 2 && (
                        <div className="text-xs text-sage-500 px-1">
          ...
          <div className="flex gap-6 mt-6 pt-4 border-t border-sage-100">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-sage-600">Agendada</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 rounded-full bg-green-500" />
              <span className="text-sm text-sage-600">Concluída</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 rounded-full bg-red-500" />
              <span className="text-sm text-sage-600">Cancelada</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 rounded-full bg-orange-500" />
              <span className="text-sm text-sage-600">Faltou</span>
            </div>
          </div>
        </motion.div>
              ...
              {selectedDayAppointments.map((apt) => (
                ...
                >
                  <div className="flex items-center gap-3 mb-2">
                    <div className={`w-2 h-2 rounded-full ${statusColors[apt.status as keyof typeof statusColors]}`} />
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
              {todayAppointments.map((apt) => (
                ...
                >
                  ...
                  <div className={`w-3 h-3 rounded-full ${statusColors[apt.status as keyof typeof statusColors]}`} />
                </Link>
              ))}
            </div>
          )
